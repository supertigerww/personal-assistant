...
def test_select_overlays_returns_multiple_phrases():
    phrases = select_humiliation_overlays("我在看av主人 寸止 不许射 漏精", count=4)
    assert 2 <= len(phrases) <= 5
    assert all(2 <= len(p) <= OVERLAY_MAX_CHARS for p in phrases)
    assert any(p for p in phrases)


def test_select_overlays_stable_with_fixed_rng():
    import random

    context = "跪着看女王训练 废物 无脑狗"
    a = select_humiliation_overlays(context, count=4, rng=random.Random(42))
    b = select_humiliation_overlays(context, count=4, rng=random.Random(42))
    assert a == b


def test_select_overlays_foot_context_prefers_foot_lines():
    phrases = select_humiliation_overlays("盯着高跟鞋 恋足 鞋底", count=4)
    blob = "".join(phrases)
    assert any(k in blob for k in ("脚", "鞋", "恋足", "丝袜", "踩"))


def test_select_overlays_allows_longer_than_ten():
    phrases = select_humiliation_overlays("锁精 盯脚 寸止", count=4)
    assert all(len(p) <= OVERLAY_MAX_CHARS for p in phrases)
    long_pool = select_humiliation_overlays("恋足 高跟鞋 鞋底 漏精", count=4)
...
def test_parse_llm_overlay_phrases_from_json():
    phrases = parse_llm_overlay_phrases(
        '["无脑狗只准看鞋尖！", "看着鞋底漏精！", "寸止！", "绿奴"]',
        count=4,
    )
...
def test_parse_llm_overlay_phrases_drops_soft_and_too_long():
    ...
    phrases = parse_llm_overlay_phrases(raw, count=4)
...
def test_normalize_overlay_phrases_dedupes():
    assert normalize_overlay_phrases(["废物", "废物", "跪好", "跪好"], count=4) == ["废物", "跪好"]
